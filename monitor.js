...
function getSystemInfo() {
  ...
  const ramUsagePercent = Math.round((ramUsage / ramTotal) * 100)
    ...
    return { 
        system, arch, cpu, uptime, ramUsage, ramTotal, ramUsagePercent
    }
}
...
function saveLog({ system, arch, cpu, uptime, ramUsage, ramTotal, ramUsagePercent }) {
    const logContent = `DETALHES DO SISTEMA | Sistema Operacional: ${system} | Arquitetura: ${arch} | 
    Modelo do Processador: ${cpu} | Tempo de Atividade do Sistema: ${uptime} | 
    Uso de Memória RAM: ${ramUsage.toFixed(2)} GB / ${ramTotal.toFixed(2)} 
    GB (${ramUsagePercent} %)\n---\n`;
